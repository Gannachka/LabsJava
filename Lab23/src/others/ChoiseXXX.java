...
public class ChoiseXXX {
    ...
    public ChoiseXXX(String d, String xxx) {
        File dir = new File(d);
        if (dir.exists()) {
            listxxx = dir.list(new OnlyXXX(xxx));//анализирует папку дир и возвращает только те файлы, где тру
        }
    }

    public static void OutputDoc(final File file, final HttpServletResponse response) {
        response.setContentType("application/msword");
        response.addHeader("Content-Disposition", format("attachment; filename=%s", file.getName()));//во второй части указываю имя файла
